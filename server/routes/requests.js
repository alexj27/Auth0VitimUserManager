...
const getAdmins = async (auth0) => {
    ...
    let ADMINS_ROLE_IDS = roles.filter(v => ADMINS_ROLES.includes(v.name)).map(v => v.id);
    ...
    if (!request) {
        ...
        if (found.length === 0) {
            request = await context.db.create('requests', {
                ...
                client_id: req.body.client_id.toLowerCase(),
                connection,
                created_at: new Date(),
...
export default (storage) => {
    ...
    const db = new tools.BlobRecordProvider(storage, { concurrentWrites: false });
    const api = Router();
    ...
    api.get('/:id', (req, res, next) => {
        db.get('requests', req.params.id)
            ...
            .catch(err => res.status(404).json({error: "Request not found"}));
    });
